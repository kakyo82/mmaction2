...
@PIPELINES.register_module()
class SampleFineEduFrames(SampleFrames):
    
    def __init__(self, clip_len, frame_interval=2, test_mode=False):

        super().__init__(clip_len, frame_interval, test_mode=test_mode)
    
    def _get_clips(self, center_index, skip_offsets):
        start = center_index - (self.clip_len // 2) * self.frame_interval
        end = center_index + ((self.clip_len + 1) // 2) * self.frame_interval
        frame_inds = list(range(start, end, self.frame_interval))
        if not self.test_mode:
    ...
    def __call__(self, results):
        fps = results['fps']
        norm_image_idx = results['norm_image_idx']

        center_index = norm_image_idx
...
class RawFrameDecode:
    ...
    def __init__(self, io_backend='disk', decoding_backend='cv2', bool_rescale_bbox = True, **kwargs):
        self.io_backend = io_backend
        self.decoding_backend = decoding_backend
        self.kwargs = kwargs
        self.file_client = None
        self.bool_rescale_bbox = bool_rescale_bbox
    ...
    def __call__(self, results):
        ...
        if 'gt_bboxes' in results and self.bool_rescale_bbox:
